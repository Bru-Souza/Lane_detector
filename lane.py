...
class Lane:
    ...
    def __init__(self, orig_frame):
        ...
        self.__init_width_height()  
        # Four corners of the trapezoid-shaped region of interest
        self.__init_roi_points()
        
        self.padding = int(0.25 * self.width) # padding from side of the image in pixels
        self.desired_roi_points = np.float32([[self.padding, 0], 
        ...
        self.margin = int((1/12) * self.width)  # Window width is +/- margin
        self.minpix = int((1/24) * self.width)  # Min no. of pixels to recenter window
            
        # Best fit polynomial lines for left line and right line of the lane
        self.__init_polynomial_variables()
        # Pixel parameters for x and y dimensions
        self.__init_pixel_parameters()
        # Radio of curvature and offset
        self.__init_radio_curvature_and_offset_variables()
    
    def __init_width_height(self):
        self.orig_image_size = self.orig_frame.shape[::-1][1:]
        self.width = self.orig_image_size[0]
        self.height = self.orig_image_size[1]

    def __init_roi_points(self):
        self.roi_points = np.float32([(400,400),(-200, 720),(1380,720),(680,400)])
        #Top-left corner, Bottom-left corner, Bottom-right corner, Top-right corner

    def __init_polynomial_variables(self):
        self.left_fit = None
        self.right_fit = None
    ...
    def __init_radio_curvature_and_offset_variables(self):
        self.left_curvem = None
        self.right_curvem = None
        self.center_offset = None
    
    def __init_pixel_parameters(self):
    ...
    def get_line_markings(self, frame=None):
        ...
        # Convert the video frame from BGR to HLS (hue, saturation, lightness).
        hls = self.__convert_rgb_to_hls(frame)

        # Isolate possible lane line edges // Perform Sobel edge detection on the L (lightness) channel of the image
        ...
        # 1s will be in the cells with the highest Sobel derivative values (strongest lane line edges)
        sxbinary = edge.mag_thresh(sxbinary, sobel_kernel=3, thresh=(110, 255))

        # Isolate possible lane lines
        # Perform binary thresholding on the S (saturation) channel 
        # A high saturation value means the hue color is pure. We expect lane lines to be nice, pure colors (i.e. solid white, yellow)
        ...
        # Bitwise AND operation to reduce noise and black-out any pixels that don't appear to be nice, pure, solid colors (like white or yellow lane lines.)       
    ...
    def __convert_rgb_to_hls(self,frame):
        return cv2.cvtColor(frame, cv2.COLOR_BGR2HLS)
  
    def calculate_car_position(self, print_to_terminal=False):
        # Assume the camera is centered in the image. Get position of car in centimeters
    ...
    def display_curvature_offset(self, frame=None, plot=False):
        ...
        curve_text = str((self.left_curvem+self.right_curvem)/2)[:7]
        center_text = str(self.center_offset)[:7]
        width_text = int((5/600)*self.width)
        width_2_text = float((0.5/600)*self.width)
        height_text1 = int((20/338)*self.height)
        height_text2 = int((20/338)*self.height)

        #cv2.putText(image_copy,'Curve Radius: '+ curve_text +' m', (width_text, height_text1), cv2.FONT_HERSHEY_SIMPLEX, (width_2_text),(255,255,255),2,cv2.LINE_AA)
        cv2.putText(image_copy,'Center difference: '+ center_text +' cm', (width_text, height_text2), cv2.FONT_HERSHEY_SIMPLEX, (width_2_text),(255,255,255),2,cv2.LINE_AA)
        if plot==True:       
    ...
    def get_lane_line_previous_window(self, left_fit, right_fit, plot=False):
        ...
        # Find the x and y coordinates of all the nonzero (white) pixels in the frame.         
        nonzero = self.warped_frame.nonzero()  
        nonzeroy = np.array(nonzero[0])
        nonzerox = np.array(nonzero[1])
                
        # Store left and right lane pixel indices
        part_one_left = (nonzerox > (left_fit[0]*(nonzeroy**2) + left_fit[1]*nonzeroy + left_fit[2] - margin))
        part_two_left = (nonzerox < (left_fit[0]*(nonzeroy**2) + left_fit[1]*nonzeroy + left_fit[2] + margin))
        left_lane_inds = (part_one_left & part_two_left) 

        part_one_right = (nonzerox > (right_fit[0]*(nonzeroy**2) + right_fit[1]*nonzeroy + right_fit[2] - margin))
        part_two_right = (nonzerox < (right_fit[0]*(nonzeroy**2) + right_fit[1]*nonzeroy + right_fit[2] + margin))
        right_lane_inds = (part_one_right & part_two_right)    
    ...
    def overlay_lane_lines(self, plot=False):
        ...
        cv2.fillPoly(color_warp, np.int_([pts]), (255,100,0))  # Deep Blue
    ...
    def perspective_transform(self, frame=None, plot=False):
        ...
        # Display the perspective transformed
        if plot == True:
           self.__plot_perspective_transform()          
        return self.warped_frame
    
    def __plot_perspective_transform(self):
